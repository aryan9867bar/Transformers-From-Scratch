...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device) : 
    ...
    while True:
        ...
        # Build mask for the tareget (decoder input)
...
def run_validation(model, validation_ds, device, tokenizer_src, tokenizer_tgt, max_len, print_msg, global_state, writer, num_examples=2) :
    model.eval()
    count = 0
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator :
            ...
            # (B, Seq_Len, Vocab_Size) -> (B, Vocab_Size, Seq_Len)
...
# Lets build the code to run this
